...
ensure_dir "vscode-projects" "${DOCKER_VOLUMES_ROOT}/vscode/projects" "${APP_UID}:${APP_GID}" "775"
ensure_dir "vscode-config" "${DOCKER_VOLUMES_ROOT}/vscode/config" "${APP_UID}:${APP_GID}" "775"
...
ensure_dir "nextcloud-data" "${DOCKER_VOLUMES_ROOT}/nextcloud/data" "${NEXTCLOUD_UID}:${NEXTCLOUD_GID}" "750"
...
ensure_dir "jellyfin-media" "${DOCKER_VOLUMES_ROOT}/jellyfin/media" "${APP_UID}:${APP_GID}" "775"
ensure_dir "qbittorrent-config" "${DOCKER_VOLUMES_ROOT}/qbittorrent/config" "${APP_UID}:${APP_GID}" "775"
ensure_dir "qbittorrent-downloads" "${DOCKER_VOLUMES_ROOT}/qbittorrent/downloads" "${APP_UID}:${APP_GID}" "775"
ensure_dir "immich-upload" "${DOCKER_VOLUMES_ROOT}/immich" "${APP_UID}:${APP_GID}" "775"
ensure_dir "immich-model-cache" "${DOCKER_VOLUMES_ROOT}/immich/model-cache" "${APP_UID}:${APP_GID}" "775"
ensure_dir "immich-postgres" "${DOCKER_VOLUMES_ROOT}/immich/postgres" "${POSTGRES_UID}:${POSTGRES_GID}" "700"
